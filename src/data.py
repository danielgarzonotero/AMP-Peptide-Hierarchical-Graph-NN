...
class GeoDatasetBase_1(InMemoryDataset):
    def __init__(self, root, raw_name, index_x, index_y_start, transform=None, pre_transform=None, **kwargs):
        self.filename = raw_name  # La ruta completa ya se proporciona
        self.df = pd.read_csv(self.filename)
        self.x = self.df[self.df.columns[index_x]].values
        self.y = self.df[self.df.columns[index_y_start]].values 
...
class GeoDataset_1(GeoDatasetBase_1):
    def __init__(self, root, raw_name, index_x, index_y_start, transform=None, pre_transform=None, **kwargs):
        super(GeoDataset_1, self).__init__(root=root, raw_name=raw_name, index_x=index_x, index_y_start=index_y_start, transform=transform, pre_transform=pre_transform, **kwargs)
...
class GeoDataset_2(GeoDatasetBase_2):
    def __init__(self, root='', raw_name='', transform=None, pre_transform=None, **kwargs):
        super(GeoDataset_2, self).__init__(root=root, raw_name=raw_name, transform=transform, pre_transform=pre_transform, **kwargs)
